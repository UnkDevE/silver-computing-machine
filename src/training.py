...
# PYTORCH CODE ONLY
def interpolate_model_train(model, train, step, bspline, names,
                            save_vid=False):
    ...
    # training loop
    epoch(model, make_spline(bspline), 5, names, train_s, test_s)
...
class GPUSplineEvaluator(Transform):
    ...
    def transform(self, x, _):
        ts = torch.stack([x.pow(n) for n in range(0, len(self.degree))])
        tms = torch.stack([(1-x).pow(n) for n in range(0, len(self.degree))])
        ...
        Pt = Pt * self.bcoeffs
        Pt = torch.permute(Pt, [i for i in range(Pt.ndim - 1, -1, -1)])
        out = self.pointmatrix(x, self.coeffs) * Pt
        return out
